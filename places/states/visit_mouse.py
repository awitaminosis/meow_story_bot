        ...
        try:
            ...
            if mouse_quest_level == 0:
                await say(
                    ...
                    chat_id,
                    [
                        "Пыхтя и фырча они пробираются через заросли лесной чащи. Потом через кусты крыжовника. Потом через овраги. Потом, уже отчаявшись найти Мышку, решают отдохнуть под кустом барбариса. Там они и встречают Мышку"
                    ],
                )
                ...
                if mouse_owl_story_stage != 4:  # noqa: PLR2004
                    await say(
                        ...
                        chat_id,
                        [
                            "Тигр и Ёжик обшаривают всю полянку, кусты барбариса, крыжовника, берёзовый бурелом, овраг, и даже ведро с червями. Мышки нигде не видно."
                        ],
                    )
                    await h_say(
                        bot,
                        chat_id,
                        [
                            "Эх, жаль мы не догадались следы сразу посмотреть, а сейчас уже всё затоптано...",
                            "Да и вечереет уже и становится плохо видно...",
                        ],
                    )
                    await t_say(
                        bot,
                        chat_id,
                        [
                            "Ёжик, что-то я волнуюсь за Мышку. Думаю, что нам надо бы её найти, мало ли чего случилось. Вот например, я помню, что в прошлый раз Мышка в теннис играла. И, смотри, ракетку не убрала, а в ёлку закинула - не похоже это на неё."
                        ],
                    )
                    await h_say(
                        bot,
                        chat_id,
                        [
                            "Я тоже так считаю. Но, думаю, что в ночной лес идти надо подготовленным. Это очень удачно, что у тебя есть светящаяся удочка. А чтобы нам самим не заблудиться, я буду отмечать дорогу с помощью червяков. Пойдём?"
                        ],
                    )
                else:
                    await say(
                        ...
                        chat_id,
                        [
                            "Рядом с тем кустом, куда была заброшена ракетка, стоит Мышка и чеканит мячик, который зачем-то привязан к полуободранной ёлке"
                        ],
                    )
                    ...
                    await t_say(
                        ...
                        chat_id,
                        [
                            "Мышка, ты тут? Мы пол леса облазили, пока тебя искали. Кажется всё в порядке. Что произошло"
                        ],
                    )
                    await m_say(
                        bot,
                    ...
                    await m_say(
                        ...
                        chat_id,
                        [
                            "Вот даже книжка есть про это приключение: https://awitaminosis.github.io/pi_meow_fir/#XXI"
                        ],
                    )
                    mouse_quest_level = 4
            ...
            if mouse_quest_level == 3:  # noqa: PLR2004
                ...
                await bot.send_message(
                    chat_id=chat_id,
                    text="А под кустом ветер перелистывает страницы Мышкиной книжки",
                    reply_markup=menu_kb,
                )
