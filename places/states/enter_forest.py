...
class EnterForest(LocationCallbackQuery):
    ...
    async def handler(self, message: Message, state: FSMContext):
        try:
            ...
            await say(bot, chat_id,["Тигр заходит в лес. Красиво зедсь, и пахнет прелыми листьями, грибами. Тут и там попадаются цветы и лесные ягоды. Кое-где виднеются раскопанные муравейники."])
            state_data = await state.get_data()
            if not state_data.get('showel_taken', False):
                await say(bot,chat_id,["О! А вот и лопата Ёжика! Да такой лопатой до пары десятков червей можно за раз накопать! И чего её Ёжик тут забыл? Возьму."])
