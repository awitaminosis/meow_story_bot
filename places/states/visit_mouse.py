    ...
    async def handler(self, message: Message, state: FSMContext):
        try:
            ...
            if mouse_quest_level == 0:
                ...
                await bot.send_message(chat_id=chat_id,
                                       text="Тигр и Ёжик при входе в лес слышат какие-то звуки лёгких ударов. Они идут на звук. Оказывается в ближайшем ельнике Мышка чеканит мячик ракеткой",
                                       )
            elif mouse_quest_level == 3:
                if mouse_owl_story_stage != 4:
                    await bot.send_message(chat_id=chat_id,
                                           text="Тигр и Ёжик обшаривают всю полянку, кусты барбариса, крыжовника, берёзовый бурелом, овраг, и даже ведро с червями. Мышки нигде не видно.",
                                           )
