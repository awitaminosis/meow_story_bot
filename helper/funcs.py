...
async def add_worms(state: FSMContext):
    try:
        ...
        worms_dig_max = (
            worms_dig_max_pcs_by_showel if showel else worms_dig_max_pcs_by_hand
        )
    ...
    try:
        ...
        if chance > 100 - WORMS_EAT_CHANCE:
            ...
            if not is_showel_mentioned:
                texts.append(
                    "Кстати, Тигр, а я тут вспомнил... Я недавно лопату забыл в лесу... Лопатой бы копать поудобнее было бы..."
                )
        ...
        if hedgehog_hints_level == 0:
            if worms >= hedgehog_hints_0_worms_requirement:
                ...
                await hw_say(
                    ...
                    chat_id,
                    [
                        "Ёжик смотрит на червяка. Червяк смотрит на Ежа. Ёжик вздыхает и говорит, что силы слишком не равны. Вот если бы червей было 10 штук..."
                    ],
                )
            ...
            if (
                ...
                await hw_say(
                    ...
                    chat_id,
                    [
                        "Я недавно видел в лесу Мышку. Тебе стоит сходить к ней - она может научить тебя плезным штукам"
                    ],
                )
                worms -= 50
                pool_fish_pcs -= 5
                ...
                await hw_say(
                    ...
                    chat_id,
                    [
                        "Тигр, вот если бы червей было штук 50...а Ещё мне нужно 5 рыб из ближайшей лужи - я опарышей хочу развести"
                    ],
                )
        else:
        ...
        if mouse_quest_level == 0:
            ...
            if river_fish_pcs >= mouse_quest_1_river_fish_requirements:
                ...
                await m_say(
                    ...
                    chat_id,
                    [
                        "Тигр, я сейчас книжку по аквариумистике дочитываю - мне совершенно необходимо 10 барбусов (речные рыбки). Обитают в Амазонке. Но вдруг и в нашей речке тоже...)"
                    ],
                )
            ...
            if sea_fish_pcs >= mouse_quest_2_sea_fish_requirements:
                ...
                await h_say(
                    ...
                    chat_id,
                    [
                        "Тигр, это хорошо, что у нас появилась такая светящаяся штука, я думаю нам она скоро понадобится"
                    ],
                )
            else:
                await m_say(
                    ...
                    chat_id,
                    [
                        "Тигр, а вот тут напиисано, что в морской рыбе содержится много фосфора. А вот в той книжке, сразу под пятном от сыра написано, как сделать фосфоресцирующую подсветку. Для этого требуется 15 морских рыб"
                    ],
                )
        else:
            await m_say(
                ...
                chat_id,
                [
                    "Мышка слишком увлечённо шуршит страницами книги и не слышит обращённые к ней вопросы"
                ],
            )
    except Exception as e:
        logger.error(f"An error occurred: {e}")
